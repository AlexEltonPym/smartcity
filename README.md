...
### TDMs (Term by Document Matrix)

ie: *yarrabiliba-docbyword.csv*

The TDMs are automatically generated by analysis programs. They are arranged with documents as columns and words as rows.

### JSONs (General document data)

ie: *processedTwitterData.json*

The JSONs are automatically generated by analysis programs. Each JSON can have multiple queries as top level keys.

Each element has at most the following attributes:

```
"text": original text of document
"location": string location ie: "sydney australia" if available
"originalLanguage": the detected source language or "en" if english
"translatedText": the translated version of the text, available even if not translated
"polarity": the sentiment value
"subjectivity": the subjectivity of the sentiment
"cleanedWords": the array of words, after text processing (ie: no stopwords, undefined words, blacklisted and query words)
```
